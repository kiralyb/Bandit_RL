...
class EnvironmentBanditsFlipsFreq:
  ...
  def __init__(
      self,
      block_flip_freq: list[int] = [60,60,30,90],
      reward_probs: list[float] = [0.9,0.7],
    ...
    # Choose a random block to start in
    self._block = 0 #np.random.binomial(1, 0.5)
    # Set up the new block
    self.new_block(0)

  def new_block(self,flipfreq,blockindex):
    """Flip the reward probabilities for a new block."""
    # Flip the block
    self._block = 1 - self._block
    if self._predif_sequence == [0]:
        if flipfreq == self._block_flip_freq[0]:
            prob = np.random.choice(self._reward_probs)
        else:
            prob = self._reward_probs[0]    
    else:
  ...
  def step(self, choice, isflip,flipfreq,blockindex):
    """Step the model forward given chosen action."""
    # Choose the reward probability associated with agent's choice
    reward_prob_trial = self.reward_probs[choice]
    ...
    # Check whether to flip the block
    if isflip:
      self.new_block(flipfreq,blockindex)
...
def run_flip_experiment(agent: Agent,
  ...
  for trial in np.arange(n_trials):
    ...
    reward = environment.step(choice,isflip,flip_freq, blockindex) 
    if isflip:
      blockindex += 1
    # Finally agent learns
    agent.update(choice, reward)
