...
class AgentQ:
  ...
  def update(self,
    ...
    self._q[1-choice] = ((1-self._forgetting_rate) * self._q[1-choice] +
...
class EnvironmentBanditsFlipsFreq:
  """Env for 2-armed bandit task with with reward probs that flip in blocks."""

  def __init__(
      self,
      block_flip_freq: list[int] = [30,60,90],
      reward_probs: list[float] = [0.9,0.9],
      predif_sequence: list[int] = [0],
      #blockindex: int = 0,
  ):
    # Assign the input parameters as properties
    self._block_flip_freq = block_flip_freq
    self._reward_probs = reward_probs
    self._predif_sequence = predif_sequence
  ...
  def new_block(self,blockindex):
    """Flip the reward probabilities for a new block."""
    # Flip the block
    self._block = 1 - self._block
    if self._predif_sequence == [0]:
        prob = np.random.choice(self._reward_probs)
    else:
        prob = self._reward_probs[self._predif_sequence[blockindex]]
    
    # Set the reward probabilites
    if self._block == 1:
      self.reward_probs = [prob, 1 - prob]
    else:
      self.reward_probs = [1 - prob, prob]

  def step(self, choice, isflip,blockindex):
    """Step the model forward given chosen action."""
    # Choose the reward probability associated with agent's choice
    reward_prob_trial = self.reward_probs[choice]
  ...
  @property
  def block_flip_freq(self) -> int:
    return self._block_flip_freq
  
  @property
  def predif_sequence(self) -> int:
    return self._predif_sequence
...
class BanditSession(NamedTuple):
  ...
  n_trials: int
  second_timeseries: Optional[np.ndarray] = None
...
def run_flip_experiment(agent: Agent,
                   environment: Environment,
                   n_trials: int) -> BanditSession:
  """Runs a behavioral session from a given agent and environment.
  ...
  flip_freq = environment.block_flip_freq[1]
  blockindex = 0
  for trial in np.arange(n_trials):
    # First record environment reward probs
    reward_probs[trial] = environment.reward_probs
    ...
    # Then environment computes a reward
    isflip = not((trial+1) % flip_freq)
    reward = environment.step(choice,isflip,blockindex)
    if isflip:
      if environment.predif_sequence == [0]:
        flip_freq = np.random.choice(environment.block_flip_freq)
      else:
        flip_freq = environment.block_flip_freq[environment.predif_sequence[blockindex]]
...
def plot_session(choices: np.ndarray,
                 ...
                 timeseries: np.ndarray,
                 timeseries_name: str,
                 timeseriesb: Optional[np.ndarray] = None,
  ...
  if labels is not None:
    ...
    if timeseries.ndim == 1:
      ax.plot(timeseries, color='black')
    else:
      ax.plot(timeseries)

  if timeseriesb is not None:
      for i in range(timeseriesb.shape[1]):    
          for j in range(timeseriesb.shape[0]):  # Iterate through each x-coordinate
                # Determine the transparency (alpha) based on the value in the timeseries
                alpha_value = timeseriesb[j, i]**4 / 0.9 / 2 # Normalize transparency
                if timeseriesb[j, 0] > timeseriesb[j, 1]: 
                  color = (0,0,1, alpha_value)
                else:
                  color = (1,0.5,0, alpha_value)

                # Create a 1x1 rectangle for each x-coordinate with variable transparency
                rect = Rectangle((j-0.5, y_low), 1, y_high - y_low, linewidth=0, facecolor=color)
                ax.add_patch(rect)  
...
def create_flip_dataset(agent: Agent,
                   environment: Environment,
                   n_trials_per_session: int,
                   n_sessions: int,
                   batch_size: Optional[int] = None):
  ...
  for sess_i in np.arange(n_sessions):
    experiment = run_flip_experiment(agent, environment, n_trials_per_session)
    experiment_list.append(experiment)
    prev_choices = np.concatenate(([0], experiment.choices[0:-1]))
    prev_rewards = np.concatenate(([0], experiment.rewards[0:-1]))
    xs[:, sess_i] = np.swapaxes(
...
class HkAgentQ(hk.RNNCore):
  ...
  Optionally add gamma forgetting to update and tau bias to decison.
  """

  def __init__(self, use_gamma=False, use_tau=False):
    super(HkAgentQ, self).__init__()

    # Haiku parameters
    alpha_unsigmoid = hk.get_parameter(
        'alpha_unsigmoid', (1,), init=hk.initializers.RandomUniform(minval=-1, maxval=1)#init=hk.initializers.Constant(0.5)
    )
    beta = hk.get_parameter(
        'beta', (1,), init=hk.initializers.RandomUniform(minval=0, maxval=2)#init=hk.initializers.Constant(1)
    )
    if use_gamma:
        gamma_unsigmoid = hk.get_parameter(
            'gamma_unsigmoid', (1,), init=hk.initializers.Constant(0.1)
        )
        self.gamma = jax.nn.sigmoid(gamma_unsigmoid)  # Ensures gamma is between (0,1)
        self.use_gamma = True  # Gamma is used
    else:
        # Check for gamma_unsigmoid without initializing it
        try:
            gamma_unsigmoid = hk.get_parameter(
                'gamma_unsigmoid', (1,),
            )
            ...
            self.use_gamma = False  # Gamma is not used

    if use_tau:
        tau = hk.get_parameter(
            'tau', (1,), init=hk.initializers.Constant(1)
        )
        self.tau = tau
        self.use_tau = True  # Gamma is used
    else:
        # Check for tau without initializing it
        try:
            tau = hk.get_parameter(
                'tau', (1,),
            )
            self.tau = tau  
            self.use_tau = True # Tau is used 
        except ValueError:
            self.use_tau = False # Tau is not used
  ...
  def __call__(self, inputs: jnp.array, prev_state: jnp.array):
    #prev_qs, prev_choice_onehot = prev_state 
    prev_qs = prev_state
    ...
    prev_onehot = jnp.roll(choice_onehot, shift=1, axis=0)  
    prev_onehot = prev_onehot.at[0].set(jnp.array([0.0, 0.0]))
    chosen_value = jnp.sum(prev_qs * choice_onehot, axis=1)  # shape: (batch_size)
    deltas = reward - chosen_value  # shape: (batch_size)

    # Create a forgetting mask for unchosen actions
    if self.use_gamma:
        chosen_update = choice_onehot * (prev_qs + self.alpha * jnp.expand_dims(deltas, -1))
        forgetting_update = (1 - choice_onehot) * ((1 - self.gamma) * prev_qs + self.gamma * self._q_init)
        new_qs = chosen_update + forgetting_update
        #prev_qs_f = (1 - self.gamma) * prev_qs + self.gamma * self._q_init
        #new_qs = prev_qs_f + self.alpha * choice_onehot * (jnp.expand_dims(reward, -1) - prev_qs_f)
    else:
        new_qs = prev_qs + self.alpha * choice_onehot * jnp.expand_dims(deltas, -1)


    if self.use_tau:
      choice_logits = self.beta * new_qs + self.tau * choice_onehot
    else:
      choice_logits = self.beta * new_qs
    
    #new_state = (new_qs, choice_onehot)
    
    return choice_logits, new_qs

  def initial_state(self, batch_size):
    values = self._q_init * jnp.ones([batch_size, 2])  # shape: (batch_size, n_actions)
    #prev_choice = jnp.zeros([batch_size, 2]) 
    return values#, prev_choice
  

class DynamicAgentQ(AgentQ):
    """
    Q-learning agent with dynamic learning rate (α)
    and dynamic inverse temperature (β),
    modulated by unexpected and expected uncertainty.
    """

    def __init__(self,
                 alpha: float = 0.2,
                 beta: float = 3.0,
                 lambda_uu: float = 0.1,   # EWMA rate for unexpected uncertainty
                 lambda_eu: float = 0.01,  # EWMA rate for expected uncertainty
                 kappa_uu: float = 2.0,    # scaling for α dynamics
                 kappa_eu: float = 1.0,    # scaling for β dynamics
                 n_actions: int = 2,
                 forgetting_rate: float = 0.,
                 perseveration_bias: float = 0.):
        ...
        self._lambda_uu = lambda_uu
        self._lambda_eu = lambda_eu
        self._kappa_uu = kappa_uu
        self._kappa_eu = kappa_eu

        # --- moving averages for uncertainty signals ---
        self._uu = 0.0   # unexpected uncertainty (|δ|)
        self._eu = 0.0   # expected uncertainty (δ²)

        # store current dynamic parameters (for logging or plotting)
        self._alpha_dyn = alpha
        self._beta_dyn = beta

    def update(self, choice: int, reward: float):
        """Update Q-values and dynamic parameters."""
        delta = reward - self._q[choice]

        # --- update uncertainty signals ---
        self._uu = (1 - self._lambda_uu) * self._uu + self._lambda_uu * abs(delta)
        self._eu = (1 - self._lambda_eu) * self._eu + self._lambda_eu * (delta ** 2)

        # --- dynamic α (unexpected uncertainty) ---
        self._alpha_dyn = np.clip(self._alpha * (1 + self._kappa_uu * self._uu), 0, 1)

        # --- dynamic β (expected uncertainty) ---
        self._beta_dyn = self._beta / (1 + self._kappa_eu * self._eu)
        self._beta = self._beta_dyn        # makes β truly dynamic in policy

        # --- forgetting for unchosen action ---
        self._q[1 - choice] = (
            (1 - self._forgetting_rate) * self._q[1 - choice]
            + self._forgetting_rate * self._q_init
        )

        # --- Q-learning update ---
        self._q[choice] = (1 - self._alpha_dyn) * self._q[choice] + self._alpha_dyn * reward

        # track last choice (if base AgentQ uses it)
        self._prev_choice = choice
